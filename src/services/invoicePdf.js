...
function renderInvoicePdf(res, data) {
  ...
  doc.font('Helvetica-Bold').fillColor(isPaid ? '#166534' : '#000000')
    .text(isPaid ? 'PAID' : formatMoney(invoice.total_cents), col2, boxTop + 46, { width: 100 })
    .fillColor('#000000');
  ...
  if (isPaid) {
    doc.fillColor('#166534').text(
      'PAID' + (invoice.payment_received_at ? ` â ${formatDate(invoice.payment_received_at)}` : ''),
      50 + colUnit - 40, y, { width: 80, align: 'right' }
    ).fillColor('#000000');
    y += 16;
  }
